...
def odopProducts(request):
    return render(request,'templates/odop.html')
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data:
                    ...
                    locations.append(location)
                    cache.set(
                        cache_key, location, timeout=None
    ...
    for location, pincode, data in zip(batch_locations, pincodes, documents):
        try:
            customIcontype = folium.CustomIcon(
                icon_image=f'static/Images/{data["Crop_Type"].strip()}.jpg',
                icon_size=(60, 60),
            )
        except Exception:
            customIcontype = folium.CustomIcon(
...
def searchBox(request):
    def searchRequst(request):
            ...
            if str(search_query).isdigit():
                search_query_org = {
                    "$or": [
                        {"contact_number": bson.int64.Int64(int(search_query))},
                        {"pincode": int(search_query)},
                        {"crop1_pincode": int(search_query)},
                        {"crop1_production": int(search_query)},
                    ]
                }
            else:
            ...
            if search_query:
                ...
                documents_2 = list(
                    collection2.sort("_id", 1).skip(start_index).limit(totalElem)
                )
                pincodes_2 = list(
                    collection.find(
                        search_query_org, {"pincode": 1, "district ": 1, "state": 1}
                    )
                    .skip(start_index)
                    .limit(totalElem)
                )
                ...
                district2 = [
                    term.get("district ", "") for term in pincodes_2
                ]  # Use get() with a default value
                state2 = [
                    term.get("state", "") for term in pincodes_2
                ]  # Use get() with a default value
                ...
                if not documents_2:
                    ...
                    totalSearchDocs = collection.count_documents(search_query_org)
                    paginator = Paginator(range(totalSearchDocs), 10)
                    page_number = request.GET.get("page", 1)
                    page_obj = paginator.get_page(page_number)

                    m = folium.Map(location=[20, 77], zoom_start=5)

                    for location2, pincode, data in zip(
                        batch_locations2, pincodes2, documents_2
                    ):
                        try:
                            customIcontype = folium.CustomIcon(
                                icon_image=f'static/Images/{data["crop_name"].strip()}.jpg',
                                icon_size=(60, 60),
                            )
                        except Exception:
                            customIcontype = folium.CustomIcon(
                                icon_image=f'static/Images/{data["crop_name"]}.jpg',
                                icon_size=(60, 60),
                            )
                        popup_text = f"""
...
def CropInsights(request):
    ...
    for location, pincode, data in zip(batch_locations2, pincodes, documents):
        try:
            if flag == 0:
                m = folium.Map(location=[location[0], location[1]], zoom_start=7)
        ...
        popup_text = f"""<div onmouseover='sayHello(event)' id='popuphtml' style='width: 200px;height:100px;overflow:scroll;scrollbar-width: none;-ms-overflow-style: none;'> Name of BC: {data.get('name_of_bc', 'N/A')}<br> Contact Number: {data.get('contact_number', 'N/A')}<br> Gender: {data.get('gender', 'N/A')}<br> Bank Name: {data.get('bank_name', 'N/A')}<br> State: {data.get('state', 'N/A')}<br> District: {data.get('district ', 'N/A')}<br> Office Name: {data.get('office_name', 'N/A')}<br> Pincode: {data.get('pincode', 'N/A')}<br> Corporate BC name: {data.get('corporate_bc_name', 'N/A')}<br> CODE WORD OF PRODUCT LIST: {data.get('code_word_of_product_list', 'N/A')}<br> CROP1 NAME: {data.get('crop1_name', 'N/A')}<br> CROP1 SEASON: {data.get('crop1_season', 'N/A')}<br> CROP1 AREA: {data.get('crop1_area', 'N/A')}<br> CROP1 PRODUCTION: {data.get('crop1_production', 'N/A')}<br> CROP1 Pincode: {data.get('crop1_pincode', 'N/A')}<br> CROP NAME: {data.get('crop_name', 'N/A')}<br> SEASON: {data.get('season', 'N/A')}<br> CROP AREA: {data.get('crop1_area', 'N/A')}<br> CROP PRODUCTION: {data.get('crop1_production', 'N/A')}<br> CROP NAME.1: {data.get('crop_name_1', 'N/A')}<br> SEASON.1: {data.get('season_1', 'N/A')}<br> CROP AREA.1: {data.get('crop_area_1', 'N/A')}<br> </div>"""

        try:
            folium.Marker(
                [location[0], location[1]],
                tooltip =data.get('odop_product', 'N/A'),
                popup=popup_text,
                icon=customIcontype,
            ).add_to(m)
        except Exception:
